...
export const sortRecipesByBestBeforeDate = (usableRecipes, fridgeItems) => {
    const recipesWithIngredientDates = usableRecipes.reduce((withDates, recipe) => {
        const ingredientsWithDates = recipe.ingredients.map(ingredient => fridgeItems.find(fridgeItem => fridgeItem.title === ingredient) || { title: ingredient });
    ...
    return recipesWithIngredientDates.sort((recipe) => {
        ...
        const someIngredientPastBestBefore = ingredientsBestBeforeDates.some(bestBeforeDate => bestBeforeDate && bestBeforeDate < today);
